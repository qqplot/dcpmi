...
class GenerationMixin:
    ...
    # added by qqplot
    def prepare_user_inputs_for_generation(self, *args, **kwargs):
        raise NotImplementedError(
            "A model class needs to define a `prepare_user_inputs_for_generation` method in order to use `.generate()`."
        )    
    ...
    def generate(
        ...
        negative_prompt_ids: Optional[torch.Tensor] = None,
        negative_prompt_attention_mask: Optional[torch.Tensor] = None,
        user_kwargs = None, # added by qqplot
        **kwargs,
        ...
        if generation_mode == GenerationMode.GREEDY_SEARCH:
            ...
            return self.beam_search(
                ...
                return_dict_in_generate=generation_config.return_dict_in_generate,
                synced_gpus=synced_gpus,
                user_kwargs=user_kwargs, # added by qqplot
                original_input_ids=inputs_tensor if model_input_name == "input_ids" else model_kwargs.pop("input_ids"), # added by qqplot
                **model_kwargs,
            )
        ...
        **model_kwargs,
        ...
        while True:
            ...
            if user_kwargs is not None:
                marginal_model_inputs = self.prepare_user_inputs_for_generation(input_ids, 
                                                                                decoder_prefix=user_kwargs["decoder_prefix"], 
            ...
            if user_kwargs is not None:
                ...
                # Hyperparameters.
                lmda, tau = user_kwargs["lmda"], user_kwargs["tau"]

                # Summarization model.
                next_token_softmax = nn.functional.softmax(next_token_logits, dim=-1)  # (batch_size * num_beams, vocab_size)
                next_token_entropy = torch.sum(-next_token_softmax * torch.log(next_token_softmax + user_kwargs["eps"]), dim=-1)
                idx_cpmi = (next_token_entropy > tau).unsqueeze(-1)
                    
                # Unconditional Prob.
                language_model = user_kwargs["language_model"]
                if language_model is None:
                    marginal_model_inputs.pop('original_input_ids', None)
                    unc_outputs = self(
                        **marginal_model_inputs,
                        return_dict=True,
                        output_attentions=False,
                        output_hidden_states=False,
                    )
                else:
                    unc_outputs = language_model(marginal_model_inputs["decoder_input_ids"])
                unc_logits = unc_outputs.logits[:, -1, :]
                next_token_unconditional_scores = nn.functional.log_softmax(unc_logits, dim=-1)
                
                # Calculate Scores.
                if user_kwargs["run_type"] == "cpmi":
                    next_token_scores = next_token_scores - lmda * idx_cpmi * next_token_unconditional_scores
                elif user_kwargs["run_type"] == "ours":                                         
                    next_token_scores = next_token_scores - lmda * idx_cpmi * next_token_unconditional_scores
                else:
                    assert True, "Please check your 'run_type' option..."
