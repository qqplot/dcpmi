...
class BartAttention(nn.Module):
    ...
    def forward(
        ...
        if (
            ...
            # cross_attentions
            # print("Size before view:", key_value_states.size())
...
class BartForConditionalGeneration(BartPreTrainedModel):
    ...
    # added by qqplot
    def prepare_user_inputs_for_generation(
        self,
        decoder_input_ids,
        past_key_values=None,
        attention_mask=None,
        ...
        **kwargs,
        ...
        r"""
        Prepare inputs as an user defined.

        - decoder_input_ids : y_{<t}        
        - decoder_prefix : (batch, seq_len_prefix)
        - decoder_input_ids: (batch * beam_size, seq_len)
        - encoder_outputs : random noise
        """
        _decoder_input_ids = decoder_input_ids.clone().detach()
        if decoder_prefix is not None:
            _decoder_input_ids = torch.cat((
                decoder_prefix[:,:-1].repeat(_decoder_input_ids.shape[0] // decoder_prefix.shape[0], 1),
                _decoder_input_ids
                ), dim=1)

        # original_input_ids: (batch_size, seq_length)
        original_input_ids[:, -1] = self.generation_config.pad_token_id # need to update
        _decoder_input_ids[:, 0]  = self.generation_config.bos_token_id
        original_input_ids = torch.cat((
            original_input_ids.repeat(_decoder_input_ids.shape[0] // original_input_ids.shape[0], 1),
            _decoder_input_ids            
            ), dim=1)
        _decoder_input_ids[:, 0] = self.generation_config.bos_token_id

        return {
            "input_ids": _decoder_input_ids,  # encoder inputs 
            "encoder_outputs": None,
            "past_key_values": None, # we don't use cache
            "decoder_input_ids": _decoder_input_ids,
            "attention_mask": None,
            "decoder_attention_mask": None,
            "head_mask": None,
            "decoder_head_mask": None,
            "cross_attn_head_mask": None, # decoder only
